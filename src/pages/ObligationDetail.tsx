...
export default function AccountDetail({ finance, accountId, onBack }: AccountDetailProps) {
  ...
  const handleSave = () => {
    ...
    if (editType === "savings") updateAccountGoal(accountId, parseMoney(editGoal));
    setEditing(false);
  ...
  return (
    ...
    >
      <div className="px-5 pt-10 pb-4 flex items-center justify-between">
        ...
        <button onClick={() => { setEditing(!editing); setEditName(account.name); setEditBalance(account.balance > 0 ? account.balance.toLocaleString("ru-RU") : "0"); setEditType(account.type); setEditGoal(account.monthlyGoal ? account.monthlyGoal.toLocaleString("ru-RU") : ""); }}

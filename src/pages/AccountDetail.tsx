...
export default function AccountDetail({ finance, accountId, onBack }: AccountDetailProps) {
  const { state, getSavingsForAccount, updateAccountBalance, updateAccountName, updateAccountType, updateAccountGoal, deleteAccount, toggleAccount } = finance;
  ...
  const handleSave = () => {
    ...
    updateAccountBalance(accountId, parseFloat(editBalance) || 0);
  ...
  return (
    ...
    >
      ...
      <div className="px-5 pt-10 pb-4 flex items-center justify-between">
        ...
        <button onClick={() => { setEditing(!editing); setEditName(account.name); setEditBalance(account.balance.toString()); setEditType(account.type); setEditGoal((account.monthlyGoal || 0).toString()); }} className="text-primary text-sm font-medium flex items-center gap-1">
      ...
      <div className="px-5 space-y-6">
        {editing ? (
          <div className="space-y-4">
            ...
            <div>
              <label className="section-header mb-1 block">Баланс</label>
              <div className="relative">
                <MoneyInput value={editBalance} onChange={setEditBalance} className="w-full glass-input px-4 py-3 text-foreground font-bold font-tabular pr-8" />
                <span className="absolute right-4 top-1/2 -translate-y-1/2 text-muted-foreground font-bold">₸</span>
              </div>
            </div>
            <div>
              <label className="section-header mb-1 block">Тип</label>
              <div className="flex gap-2">
                {(["active", "savings", "inactive"] as AccountType[]).map((t) => (
                  <button key={t} onClick={() => setEditType(t)}
                    className={`flex-1 py-2.5 rounded-xl text-sm font-semibold transition-colors ${
                      editType === t ? "bg-primary text-white" : "bg-card text-muted-foreground"
                    }`}>
            ...
            {editType === "savings" && (
              <div>
                <label className="section-header mb-1 block">Цель в месяц</label>
                <div className="relative">
                  <MoneyInput value={editGoal} onChange={setEditGoal} className="w-full glass-input px-4 py-3 text-foreground font-bold font-tabular pr-12" />
          ...
          <>
            ...
            {account.type === "savings" && goal > 0 && (
              <div className="bg-card rounded-2xl p-4 space-y-2">
                ...
                <div className="flex justify-between text-xs text-muted-foreground">
                  <span>Отложено: <span className="text-primary font-semibold">{formatAmount(saved)} ₸</span></span>
                  <span>{pct}%</span>
                </div>
              </div>
            )}
            ...
            <div>
              <h3 className="section-header mb-3">Операции по счёту</h3>
                    ...
                    <div key={date}>
                      ...
                      <div className="bg-card rounded-2xl overflow-hidden">
                        {txns.map((t, i) => {
                          ...
                          return (
                            <div key={t.id} className={`px-4 py-3 flex items-center justify-between ${i < txns.length - 1 ? "border-b border-border" : ""}`}>

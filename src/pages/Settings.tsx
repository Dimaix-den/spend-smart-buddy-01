  ...
  return (
    <div className="flex flex-col min-h-screen pb-28">
      ...
      <div className="flex-1 px-5 space-y-6">
        {/* Account */}
        <div>
          <h2 className="section-header mb-3 px-1">Аккаунт</h2>
          <div className="bg-card rounded-2xl p-4 space-y-3">
            ...
            <button onClick={onLogout}
              className="w-full flex items-center justify-center gap-2 py-2.5 rounded-xl text-sm font-semibold text-destructive transition-colors bg-destructive/5 hover:bg-destructive/10">
        ...
        {/* Budget Period */}
        <div>
          <h2 className="section-header mb-3 px-1">Период бюджета</h2>
          <div className="bg-card rounded-2xl p-4 space-y-4">
            <button onClick={() => handlePeriodChange("calendar")} className="w-full flex items-start gap-3 text-left">
              <div className="w-5 h-5 rounded-full border-2 flex-shrink-0 flex items-center justify-center mt-0.5"
                style={{ borderColor: periodType === "calendar" ? "hsl(var(--primary))" : "hsl(var(--border))" }}>
                {periodType === "calendar" && <div className="w-2.5 h-2.5 rounded-full bg-primary" />}
              </div>
              <div>
            ...
            <button onClick={() => handlePeriodChange("salary")} className="w-full flex items-start gap-3 text-left">
              <div className="w-5 h-5 rounded-full border-2 flex-shrink-0 flex items-center justify-center mt-0.5"
                style={{ borderColor: periodType === "salary" ? "hsl(var(--primary))" : "hsl(var(--border))" }}>
                {periodType === "salary" && <div className="w-2.5 h-2.5 rounded-full bg-primary" />}
              </div>
              <div>
                  ...
                  {days.map((d) => (
                    <button key={d} onClick={() => handleDayChange(d)}
                      className={`flex-shrink-0 w-10 h-10 rounded-xl text-sm font-semibold transition-all ${
                        salaryDay === d ? "bg-primary text-white" : "bg-background text-muted-foreground"
                      }`}>
                      {d}
                    </button>
                  ))}
        ...
        <div>
          <h2 className="section-header mb-3 px-1">Расчёты</h2>
        ...
        {/* Currency */}
        <div>
          <h2 className="section-header mb-3 px-1">Валюта</h2>
          <div className="bg-card rounded-2xl p-4">
            <div className="flex items-center justify-between">
        ...
        {/* About */}
        <div>
          <h2 className="section-header mb-3 px-1">О приложении</h2>
